...
export default function FeeAddForm({
  ...
  // Load class template + previous dues when student changes
  function loadForStudent(studentId) {
    ...
    // Template: prefer class package items, fallback to fee_structures
    ...
    if (pkg && pkg.items.length > 0) {
      ...
      for (const fs of feeStructures.filter((f) => f.class === student.class)) {
        newAmounts[fs.fee_type] = String(fs.amount);
        newChecked[fs.fee_type] = true;
      }
    }
  ...
  const total = useMemo(() => {
    ...
    return (previousDues || 0) + reg * monthCount + occ;
  }, [checkedTypes, amounts, selectedMonths, occasionalMonths, previousDues]);
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3 flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-indigo-500 mt-0.5">
                Previous dues + {selectedMonths.length} month(s) + extra
              </p>
            </div>
