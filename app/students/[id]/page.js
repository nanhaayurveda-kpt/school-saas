    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4 flex items-center gap-4">
        ...
        <div>
          ...
          <span className={`inline-block mt-1 px-2 py-0.5 text-xs rounded-full font-medium ${s.fee_status === "paid" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
            {s.fee_status}
          </span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        {concessions.length > 0 ? (
          <div className="space-y-2 mb-4">
            {concessions.map((c) => (
              <div key={c.id} className="flex justify-between items-center bg-green-50 border border-green-100 rounded-lg px-3 py-2.5">
                <div>
                  <p className="text-sm font-medium text-gray-900">
        ...
        <form action={addConcession} className="space-y-3">
          ...
          <div>
            ...
            <input type="text" name="reason" placeholder="e.g. Staff child, Merit, Poor family"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <button type="submit"
